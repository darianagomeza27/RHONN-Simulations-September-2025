...
def rhonn_predict(z, weights):
    # Nonlinear activation (Hyperbolic Tangent)
    # Predicts velocity/increment, scaled by 2.0 to allow sufficient range
    # y = 2.0 * tanh(w^T z)
    raw = np.dot(weights, z)
    return 2.0 * np.tanh(raw)

def rhonn_predict_jacobian(z, weights):
    # Derivative of y = S * tanh(u) w.r.t w is S * (1 - tanh^2(u)) * z
    raw = np.dot(weights, z)
    activation = np.tanh(raw)
    derivative = 1.0 - activation**2
    return 2.0 * derivative * z
...
class EKF_RHONN_Trainer:
    ...
    def get_estimate(self, chi_k, x_hat_prev, u_k):
        ...
        deltas = np.array([rhonn_predict(z, self.weights[i]) for i in range(3)])
        return chi_k + deltas
...
class PF_RHONN_Trainer:
    ...
    def update(self, chi_kp1, chi_k, u_k, x_hat_prev):
        ...
        for i in range(len(self.particles)):
            ...
            # preds = rhonn_predict(z, particles) -> Need to vectorize or loop
            # rhonn_predict is: 2.0 * tanh(w @ z)
            # particles[i] is (N, features)
            # z is (features,)
            # dot is (N,)
            raw_preds = np.dot(self.particles[i], z)
            preds = 2.0 * np.tanh(raw_preds)
...
# ==========================================
# 4.5 UKF Trainer (Unscented Kalman Filter)
# ==========================================
class UKF_RHONN_Trainer:
    ...
    def generate_sigma_points(self, w, P):
        ...
        # Square root of P
        # Using Cholesky decomposition. P must be positive definite.
        try:
    ...
    def update(self, chi_kp1, chi_k, u_k, x_hat_prev):
        ...
        for i in range(self.num_neurons):
            ...
            # w_k|k-1 = w_k-1|k-1 (Random walk model)
            ...
            # Propagate sigma points through measurement function (RHONN)
            # y = rhonn_predict(z, w)
...
def run_simulation():
    ...
    fig = make_subplots(rows=2, cols=1, 
                        subplot_titles=("2D Trajectory: EKF vs UKF vs PF in Laplacian Noise", "Mean Squared Error over Time"),
    ...
    fig.add_trace(go.Scatter(x=h_ukf[:, 0], y=h_ukf[:, 1], mode='lines', name='UKF (Unscented)', line=dict(color='orange', width=1, dash='dashdot')), row=1, col=1)
    ...
    fig.add_trace(go.Scatter(y=err_ukf, mode='lines', name=f'UKF MSE (Mean: {np.mean(err_ukf):.5f})', line=dict(color='orange', width=1)), row=2, col=1)
